/* USF file - DO NOT MODIFY THIS FILE. THIS FILE IS REGULARLY CHANGED BY USF APP AND **ANY DIRECT CHANGES WILL BE LOST**. Use our in-app customization if you need to update CSS and JS code. Auto modified at: 9/16/2023 1:54:40 AM*/
...
<!-- List + Swatch filter -->
<div v-else-if="facet.title=='Color' && !usf.isMobile" ref="values" :class="'usf-colorfilter usf-facet-values usf-scrollbar usf-facet-values--' + facet.display +  (facet.navigationCollections ? ' usf-navigation' : '') + (facet.valuesTransformation ? (' usf-' + facet.valuesTransformation.toLowerCase()) : '') + (facet.circleSwatch ? ' usf-facet-values--circle' : '')" :style="!usf.isMobileFilter && facet.maxHeight ? { maxHeight: facet.maxHeight } : null">
     <!-- Filter options -->                        
    <div v-for="o in visibleOptions" class="usfswatch"> 
        <usf-filter-option :facet="facet" :option="o" :key="o.label"></usf-filter-option>  
      <!--   <span v-html="o.label" class="usf-color-name"></span> -->    
    </div>  
</div>
...
<div v-else-if="facet.facetName=='option:Color' && !usf.isMobile" @click.prevent.stop="onToggle" class="usfswatch">
    <span v-html="label" class="usf-color-name"></span>
    <button :aria-label="label != '' ? label : 'button'" :class="(isSelected ? 'usf-selected ' : '') + (swatchImage ? ' usf-facet-value--with-background' : '') + ' usf-btn usf-relative usf-facet-value usf-facet-value-' + (facet.multiple ? 'multiple' : 'single')" :title="isSwatch || isBox ? label + ' (' + option.value + ')' : undefined" :style="usf.isMobileFilter ? null : swatchStyle" >
        <!-- checkbox -->
